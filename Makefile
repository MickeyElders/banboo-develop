...
build-engine:
	@if [ -f models/best.engine ]; then \
		...
		"$(TRTEXEC)" --onnx=models/best.onnx --saveEngine=models/best.engine --fp16 --best --builderOptimizationLevel=5 --noBuilderCache --memPoolSize=workspace:$(TRT_WORKSPACE) || true; \
