...
bool DeepStreamRunner::buildServer(const std::string &launch) {
    ...
    // Share a single pipeline across clients to avoid multiple nvarguscamerasrc instances (Argus is exclusive).
...
bool DeepStreamRunner::buildWebRTCPipeline() {
    ...
    // Build rtspsrc -> depay -> queue -> parse -> queue -> pay -> caps -> webrtcbin
